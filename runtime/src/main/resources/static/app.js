  ...
  tb.innerHTML = list.map(p => {
    ...
    return `<tr>
      ...
      <td style="white-space:nowrap">
        ${toggleBtn}
        <button id="btn-export-${esc(p.id)}" class="btn" style="background:rgba(100,116,139,.2);border:1px solid #475569;color:#94a3b8;font-size:11px" onclick="downloadProbeContainer('${esc(p.id)}')">⬇ Edge ZIP</button>
...
// ── Edge export ────────────────────────────────────────────────────────
async function downloadProbeContainer(probeId) {
  const btn = document.getElementById('btn-export-' + probeId);
  if (btn) { btn.disabled = true; btn.textContent = '⏳ Building...'; }
  try {
    const response = await fetch(`/api/probes/${encodeURIComponent(probeId)}/export`, {
    ...
    const filename = match ? match[1] : `concern-probe-${probeId}.zip`;
    ...
    if (btn) { btn.disabled = false; btn.textContent = '⬇ Edge ZIP'; }
  }
}
