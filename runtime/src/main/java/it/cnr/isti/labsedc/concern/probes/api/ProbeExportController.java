...
public class ProbeExportController {
    ...
    private String buildEnv(ProbeDefinition def) {
        ...
        return "# ── Set BROKER_URL to the ActiveMQ broker address reachable from this edge node ──\n" +
    ...
    private String buildReadme(ProbeDefinition def) {
        ...
        return "# Probe: " + def.name + " (`" + def.id + "`)\n\n" +
               "Package generated by concern-probes-runtime.\n" +
               "Contains everything needed to run this probe on an edge node with Docker.\n\n" +
               "## Prerequisites\n" +
               "Docker + Docker Compose installed on the edge node.\n\n" +
               "## Deploy\n\n" +
               "```bash\n" +
               "# 1. Extract\n" +
               "unzip concern-probe-" + def.id + "-*.zip\n" +
               "cd concern-probe-" + def.id + "-*/\n\n" +
               "# 2. Set the ActiveMQ broker address reachable from this node\n" +
               "nano .env   # edit BROKER_URL\n\n" +
               "# 3. Start\n" +
               ...
               "```\n\n" +
               "## Notes\n" +
               "- Only `BROKER_URL` in the `.env` file needs to be changed.\n" +
               "- If the probe configuration is wrong, regenerate it from the main runtime and re-download.\n" +
               "- The edge runtime has no UI or REST API: it produces events and sends them to the broker.\n" +
               "- If the broker is unreachable, the edge runtime retries with exponential backoff\n" +
               "  (2s → 4s → 8s … max 60s) without losing events.\n\n" +
               "## Probe details\n" +
               "| Field | Value |\n" +
               "|---|---|\n" +
               "| ID | `" + def.id + "` |\n" +
